...
int cubacall::nst;
int cubacall::last_iter;
void cubainit()
{
  cubacall::nst = 10;
  cubacall::last_iter = 4;

  //fully correlated variations with vegas
  if (opts.vegascorr)
    {
      cubacall::nst = 1;
      cubacall::last_iter = 1;
    }
}


// -------------------------------------------------------------------
// -- Reals diphoton NLO QCD -- Lean 31/05/2025
// -------------------------------------------------------------------
void nlorealqqqcdintegrMC8d(vector <double> &res, double &err)
{ 
  ...
  const int mineval   = opts.vegasncallsNLO;
  const int maxeval   = opts.vegasncallsNLO;
  const int nstart    = max(10, int(opts.vegasncallsNLO/10));
  const int nincrease = max(10, int(opts.vegasncallsNLO/10));
  const int nbatch    = opts.cubanbatch;
  const int gridno = 0;

  Vegas(ndim, ncomp, (integrand_t) nlorealqqqcdintegrandMC, userdata, nvec,
        epsrel, epsabs,
        flags, seed,
        mineval, maxeval,
...
// -------------------------------------------------------------------
// -- Reals diphoton NLO EW -- Lean Junio 2025
// -------------------------------------------------------------------
void nlorealEWintegrMC8d(vector <double> &res, double &err)
{ 
  const int ndim = 8;   //dimensions of the integral
  const int ncomp = opts.totpdf;  //components of the integrand
  void *userdata;
  ...
  Vegas(ndim, ncomp, (integrand_t) nlorealEWintegrandMC, userdata, nvec,
        epsrel, epsabs,
        flags, seed,
        mineval, maxeval,
        nstart, nincrease, nbatch,
...
/***************************************************************/
//resummation
void resintegr0d(vector <double> &res, double &err)
{
...
void resintegrMC(vector <double> &res, double &err)
{
  ...
  const int nstart = max(10, int(opts.vegasncallsBORN/cubacall::nst));
...
void vjlointegr(vector <double> &res, double &err)
{
  ...
  const int nstart    = max(10, int(opts.vegasncallsVJLO/cubacall::nst));
...
void vjlointegr0d(vector <double> &res, double &err)
{
...
void vjrealintegr(vector <double> &res, double &err)
{
  ...
  const long long int nstart = max(10, int(opts.vegasncallsVJREAL/cubacall::nst));
...
void vjvirtintegr(vector <double> &res, double &err)
{
  ...
  const int nstart = max(10, int(opts.vegasncallsVJVIRT/cubacall::nst));
...
void v2jintegr(vector <double> &res, double &err)
{
  ...
  const int nstart = max(10, int(opts.vegasncallsVJREAL/cubacall::nst));
...
void ctintegr(vector <double> &res, double &err)
{
  ...
  const int nstart = max(10, int(opts.vegasncallsCT/cubacall::nst));
...
void fpcintegr2d(vector <double> &res, double &err)
{
  const int ndim = 2;     //dimensions of the integral
  const int ncomp = opts.totpdf;  //components of the integrand
  void *userdata = NULL;
  ...
  if (!opts.pcubature)
    Cuhre(ndim, ncomp,
	  (integrand_t) fpcintegrand2d, userdata, nvec,
	  epsrel, epsabs,
	  flags,
	  mineval, maxeval,
	  key, statefile, NULL,
  ...
  else
    {
      ...
      double xmax[2] = {1, 1.};
      if (opts.cubacores == 0)
	pcubature(ncomp, fpcintegrand2d_cubature, userdata, 
		  ndim, xmin, xmax, 
		  eval, epsabs, epsrel, ERROR_INDIVIDUAL, integral, error);
      else
	pcubature_v(ncomp, fpcintegrand2d_cubature_v, userdata, 
		    ndim, xmin, xmax, 
		    eval, epsabs, epsrel, ERROR_INDIVIDUAL, integral, error);
    }
...
//Cuba integration of the FPC term
void fpcintegrMC(vector <double> &res, double &err)
{
  const int ndim = 8;   //dimensions of the integral
  const int ncomp = opts.totpdf;  //components of the integrand
  void *userdata;
  ...
  const int nbatch = opts.cubanbatch;
  const int gridno = 0;
  Vegas(ndim, ncomp, (integrand_t)fpcintegrandMC, userdata, nvec,
	epsrel, epsabs,
	flags, seed,
	mineval, maxeval,
	nstart, nincrease, nbatch,
...
//Integration of CT+VJ
void fointegr0d(vector <double> &res, double &err)
{
  const int ndim = 3;     //dimensions of the integral
  const int ncomp = 1;    //components of the integrand
  double integral[ncomp];
  ...
  int print_stats = 0;
  int dim = ndim;
  int l = 3; //maximum 23, take from settings
  integral[0] = int_smolyak (ndim, ndim+l, fointegrand3d_smolyak, print_stats );
  error[0] = 0.000000001;
  */
  //sparse grids
  integral[0] = fointegrand0d();
  error[0] = 0.000000001;
...
void fointegr3d(vector <double> &res, double &err)
{
  const int ndim = 3;     //dimensions of the integral
  const int ncomp = 1;    //components of the integrand
  void *userdata;
  ...
  if (!opts.pcubature)
    Cuhre(ndim, ncomp,
	  (integrand_t) fointegrand3d, userdata, nvec,
	  epsrel, epsabs,
	  flags,
	  mineval, maxeval,
	  key, statefile, NULL,
  ...
  else
    {
      ...
      double xmax[3] = {1, 1., 1.};
      if (opts.cubacores == 0)
	pcubature(ncomp, fointegrand3d_cubature, userdata,
		  //hcubature(ncomp, fointegrand3d_cubature, userdata, 
		  ndim, xmin, xmax, 
		  eval, epsabs, epsrel, ERROR_INDIVIDUAL, integral, error);
      else
	pcubature_v(ncomp, fointegrand3d_cubature_v, userdata, 
		    ndim, xmin, xmax, 
		    eval, epsabs, epsrel, ERROR_INDIVIDUAL, integral, error);
      /*
      //smolyak
...
/***************************************************************/
void tell_to_grid_we_are_alive(int step){
  if(opts.gridverbose && ICALL % step==0)
    {
      ...
      if (opts.redirect) freopen((opts.output_filename + ".log").c_str(), "a", stdout);
    }
  ICALL++;
}
